...
class NaiveBayes:
    ...
    def PredictLabel(self, X, threshold=0.5):
        ...
        self.P_positive = log(self.num_positive_reviews) - (
                    log(self.num_positive_reviews) + log(self.num_negative_reviews))
        self.P_negative = log(self.num_negative_reviews) - (
                    log(self.num_positive_reviews) + log(self.num_negative_reviews))
        pred_labels = []
        w = X.shape[1]
        sh = X.shape[0]
...
if __name__ == "__main__":
    ...
    traindata = IMDBdata("C:/Users/Cosmic Travellers/PycharmProjects/AI_NB/IMDB_DataSets/train")
    print("Reading Test Data")
    testdata = IMDBdata("C:/Users/Cosmic Travellers/PycharmProjects/AI_NB/IMDB_DataSets/test", vocab=traindata.vocab)

    print("Computing Parameters")
    nb = NaiveBayes(traindata, 1)      # float(sys.argv[2])
    print("Evaluating")
    nb.Eval(testdata)
    nb.PredictProb(testdata, range(10))
    nb.Features()

    print("Computing Parameters 0.5")
    nb = NaiveBayes(traindata, 0.5)  # float(sys.argv[2])
    print("Evaluating")
    nb.Eval(testdata)
    nb.PredictProb(testdata, range(10))
    nb.Features()

    print("Computing Parameters 0.1")
    nb = NaiveBayes(traindata, 0.1)  # float(sys.argv[2])
    print("Evaluating")
    nb.Eval(testdata)
    nb.PredictProb(testdata, range(10))
    nb.Features()
